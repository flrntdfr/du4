...
compress() {
  command -v gs >/dev/null 2>&1 || {
    echo "Error: ghostscript (gs) is required for compress." >&2
    exit 1
  }
...
extract_img() {
  command -v pdfimages >/dev/null 2>&1 || {
    echo "Error: pdfimages is required for extract-img." >&2
    exit 1
  }
  mkdir -p "target"
...
encrypt() {
  ...
  command -v qpdf >/dev/null 2>&1 || {
    echo "Error: qpdf is required for encrypt." >&2
    exit 1
  }
...
case "$subcommand" in
  compress)
    ...
    [ -f "$in_file" ] || { echo "Error: file '$in_file' not found." >&2; exit 1; }
    compress "$in_file" "$dpi" "$out_file"
    ;;
  encrypt)
    [ "${1:-}" = "-p" ] || { echo "Error: encrypt requires -p <password>." >&2; exit 1; }
    password="${2:-}"
    in_file="${3:-}"
  ...
  fonts)
    ...
    ;;
  stamp|embed|extract)
    echo "Error: subcommand '$subcommand' is not implemented yet." >&2
    exit 1
    ;;
  *)
